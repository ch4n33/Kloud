...
# 영속 디렉토리 보장 (state PV의 subPath들이 첫 부팅 시 비어 있을 수 있음)
mkdir -p /claude-home/.local/bin /opt/cli/bin /app/data
...
# 첫 부팅 시 compact-bot init (data/messages.json 생성)
if [ ! -f /app/data/messages.json ]; then
  echo "[entrypoint] compact-bot data/ 초기화"
  cd /app && yes '' | npx --yes @serin511/compact-bot init || true
fi
